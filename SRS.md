...
Fixed 2026-09-22 - a real gap on web only, found on review, not caused by the fix above: `webNotificationStrategy.getDeviceToken()` (`lib/permission-strategies/web.ts`) called `getWebPushToken()` (`lib/firebase-web.ts`) unconditionally, with no check of the browser's current permission state first. `components/DeviceTokenSync.tsx` calls this on every dashboard mount, and `components/SettingsToggles.tsx`'s `refreshPermissionState` calls it on every Settings mount and every focus/visibility regain (both unconditional too, regardless of what `isGranted()` itself returns) - so both run well before a customer has ever seen the in-app "Turn on notifications" primer. The installed Firebase SDK's `getToken()` (`@firebase/messaging`) itself calls `Notification.requestPermission()` whenever `Notification.permission === "default"`, i.e. it asks the browser to show its native permission dialog on the app's behalf. Practical effect: the browser's own permission prompt could appear unprompted on a customer's very first dashboard visit, with none of the primer's explanatory UI - and if reflexively dismissed or blocked (likely, given no context), `Notification.permission` becomes `"denied"` for that origin, which browsers do not allow re-prompting for. From then on, the real primer's own `requestPermission()` call (`handleNotificationAllow`) would silently resolve `false` with no dialog at all, showing "Notifications may already be blocked for this site" with no in-app recovery - permanently short-circuiting that customer's web opt-in path, the opposite failure to the one FR-2.9's own reverted-regression note describes. Fixed by adding the same `Notification.permission === "granted"` guard `isGranted()` already uses to the top of `getDeviceToken()` itself - the single call site both components go through - so a token is only ever fetched/refreshed once permission was already granted via the primer, never used to ask for it. Confirmed against the installed `@firebase/messaging@0.13.2` source (`getToken$1` in `dist/esm/index.esm.js`) that this is exactly where the auto-request happens. Verified by type-check and lint only, not yet exercised in a live browser against a fresh (never-asked) origin - see [Section 13](#13-known-limitations-and-deferred-work).

Files: `lib/permission-strategies/`, `components/SettingsToggles.tsx`, `components/DeviceTokenSync.tsx`, `lib/firebase-web.ts`
...
- **FR-2.5's 2026-09-22 fix for the browser's unsolicited-permission-prompt bug has not been verified in a live browser.** Confirmed by reading the installed `@firebase/messaging` SDK source and by type-check/lint only - not yet exercised against a real browser profile with `Notification.permission` still at `"default"`, to confirm the fix actually suppresses the auto-prompt on first dashboard visit rather than merely type-checking. See FR-2.5's own note for the bug and fix. Also unconfirmed: whether the deployed environment (`expwebapp.vercel.app`) has `NEXT_PUBLIC_FIREBASE_VAPID_KEY` and the rest of the Firebase web config set at all - present in `.env.local` for local dev, but not checked against the deployed environment's own variables, so it's unknown whether web push (and thus this bug, and its fix) is even reachable there yet.
- **The Android emulator available for device testing is configured against the deployed `https://expwebapp.vercel.app/` origin, not a local dev server - discovered 2026-09-18, after a long debugging session had wrongly assumed otherwise.** `fog-mobile-app`'s built-and-installed `android/agua` app has `capacitor.config.json`'s `server.url` pointed at that deployed origin; nothing in `exp-webapp`'s own working tree configures this, since that setting lives entirely in the other repo's build output. Practically: on-device/emulator testing only ever exercises whatever is currently deployed there, never uncommitted local changes to `exp-webapp` - confirmed the hard way while chasing what looked like a reconciliation bug (see FR-2.9's third revision note) that turned out to be untestable on that emulator for this reason, not a confirmed defect in the fix. Retesting local `exp-webapp` changes on-device needs either a real deploy first, or reconfiguring/rebuilding `fog-mobile-app` to point at a local server (the `10.0.2.2` emulator-to-host alias `next.config.ts` already allows for) - neither was done here. A claim of "verified on device" for anything in this document should be read as verifying whatever was deployed at the time, not necessarily the exact diff being described immediately around it, unless that diff is explicitly noted as already deployed.
